...
lmer(length_sum~day+dist_road_resc+temp_sc+wind_sc+humidity_sc+pressure_sc+cloudiness_sc+(1|locality/code),
     data=df[df$loc=="AL",]) %>% 
  vif # humidity will be removed
lmer(length_sum~day+dist_road_resc+temp_sc+wind_sc+pressure_sc+cloudiness_sc+(1|locality/code),
     data=df[df$loc=="AL",]) %>% 
  vif # fine

# model selection
lmerTest::ranova(lmerTest::lmer(length_sum~day*(dist_road_resc+temp_sc+wind_sc+pressure_sc+cloudiness_sc)+(1|locality/code),
                                data=df[df$loc=="AL",]))
...
# model selection
lmerTest::ranova(lmerTest::lmer(length_mean~day*(dist_road_resc+temp_sc+wind_sc+pressure_sc+cloudiness_sc)+(1|locality/code),
                                data=df[df$loc=="AL",]))
...
# model selection
lmerTest::ranova(lmerTest::lmer(singing_sunrise~day*(dist_road_resc+temp_sc+wind_sc+pressure_sc+cloudiness_sc)+(1|locality/code),
                                data=df[df$loc=="AL",]))
m2.3 <- lmer(singing_sunrise~day*(dist_road_resc+temp_sc+wind_sc+pressure_sc+cloudiness_sc)+(1|locality/code),
             data=df[df$loc=="AL",])
m2.3 <- stepAIC(m2.3)
plot(m2.3)
qqmath(m2.3)
