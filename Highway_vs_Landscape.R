...
lmer(length_sum ~ day+loc+temp_sc+wind_sc+humidity_sc+pressure_sc+cloudiness_sc+(1|code)+(1|locality), data=df) %>% 
  vif # humidity will be removed
lmer(length_sum ~ day+loc+temp_sc+wind_sc+pressure_sc+cloudiness_sc+(1|code)+(1|locality), data=df) %>% 
  vif # fine

# model selection
lmerTest::ranova(lmerTest::lmer(length_sum ~ day*loc*(temp_sc+wind_sc+pressure_sc+cloudiness_sc)+(1|code)+(1|locality), data=df))
